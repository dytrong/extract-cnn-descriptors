...
def points_distance(pt1,pt2,H_path):
    ...
    L2_dis=compute_max_match_nn(pt1,pt2) 
    valid_match=[]
    #####找出实际真正的匹配对
...
def compute_valid_match(max_match,Thresh):
    valid_match=[]
    for i in range(len(max_match)):
        if max_match[i][2]>=Thresh:
            valid_match.append(max_match[i])
    valid_match=np.array(valid_match)
    return valid_match

def compute_PR(max_match,thresh,ground_true_match):
    max_match=compute_valid_match(max_match,thresh)
...
def match(img1,img2,kp1,kp2,max_match,H_path,imshow=False):
    if imshow:
        #####显示匹配对
        ...
        max_list.sort()
        Thresh=max_list[-50]
        p,r,valid_match,correct_match=compute_PR(max_match,Thresh,ground_true_match)
        print('正确率为:'+str(p))
        show_keypoints(kp1,kp2,img1,img2,valid_match,correct_match) 
    else:
        ...
        for i in range(step_number):
            ...
            precision,recall=compute_PR(max_match,thresh,ground_true_match)
            P_list.append(precision)
            R_list.append(recall)
        #print('精度为:'+str(P_list))
        #print('召回率为:'+str(R_list))
...
def show_keypoints(kp1,kp2,img1,img2,max_match,correct_match):
    cols1 = img1.shape[1]
    ...
    for i in range(len(correct_match)):
        key_points1.append(kp1[int(correct_match[i][0])])
        key_points2.append(kp2[int(correct_match[i][1])]) 
    for i in range(len(max_match)):
    ...
    for i in range(len(kp_1)):
        ...
        if kp_1[i] in key_points1:
            cv2.line(img3, (int(np.round(x1)),int(np.round(y1))), (int(np.round(x2)+cols1),int(np.round(y2))), (255, 0, 0), 1, lineType=cv2.LINE_AA, shift=0)
        else:
            cv2.line(img3, (int(np.round(x1)),int(np.round(y1))), (int(np.round(x2)+cols1),int(np.round(y2))), (0, 0, 255), 1, lineType=cv2.LINE_AA, shift=0)
