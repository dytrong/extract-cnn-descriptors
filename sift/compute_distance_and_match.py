...
def compute_des_L2_dis(des1,des2):
    ...
    des1=np.sqrt(preprocessing.normalize(des1,norm='l1'))
    des2=np.sqrt(preprocessing.normalize(des2,norm='l1'))
...
####计算坐标的欧式距离，和计算描述符的欧式距离一样，但是
####不能对它进行开平方根，坐标可能为负
def compute_axis_L2_dis(des1,des2):
    des1=torch.from_numpy(des1).cuda(device) ####change numpy.array to torch.tensor and change cpu data to gpu data by use .cuda()
    des2=torch.from_numpy(des2).cuda(device)
    pdist=torch.nn.PairwiseDistance(2)
    dis_matrix=np.zeros((len(des1),len(des2)))
...
def compute_pixs_distance(des1,des2):
    cos_dis=compute_axis_L2_dis(des1,des2)    ####L2_dis
...
#########通过H矩阵算左图到右图对应位置
def compute_corresponse(pt1,pt2,H_path):
    H=np.loadtxt(H_path,dtype=np.float64)
    pt2_list=[]
    pt2_array=np.array(pt2)
    pt2_x_max=np.max(pt2_array[:,0])
    pt2_y_max=np.max(pt2_array[:,1])
    print('pt2 x最大值为:'+str(pt2_x_max))
    print('pt2 y最大值为:'+str(pt2_y_max))
...
def points_distance(pt1,pt2,H_path):
    #####计算图1进过H变换后在图2中的估计位置
    pt1=compute_corresponse(pt1,pt2,H_path)
    ######将Point类型变量,转化为2维数组,方便计算L2 distance
    pt1=np.array(pt1)
    pt2=np.array(pt2)
...
def match(img1,img2,kp1,kp2,max_match,H_path,imshow=True,method='L2'):
    if imshow:
        #####显示匹配对
        ...
        p,r,correct_match,detect_match=compute_PR(max_match,Thresh,ground_true_match,imshow,method)
        print('正确率为:'+str(p))
        show_keypoints(kp1,kp2,img1,img2,correct_match,detect_match) 
    else:
...
def show_keypoints(kp1,kp2,img1,img2,correct_match,detect_match):
    cols1 = img1.shape[1]
    img3=append_image(img1,img2)
    if len(detect_match)==0:
        return
    key_points1=[]
    key_points2=[]
    kp_1=[]
    kp_2=[]
    #####写入所有算法上认为是匹配上的点对
    for i in range(len(detect_match)):
        key_points1.append(kp1[int(detect_match[i][0])])
        key_points2.append(kp2[int(detect_match[i][1])]) 
    ####检测出实际为正确的匹配点对
    for i in range(len(correct_match)):
        kp_1.append(kp1[int(correct_match[i][0])])
        kp_2.append(kp2[int(correct_match[i][1])])
    for i in range(len(key_points1)):
        (x1,y1)=key_points1[i]
        (x2,y2)=key_points2[i]
        cv2.circle(img1,(int(np.round(x1)),int(np.round(y1))),5,(255,0,0),6)
        cv2.circle(img2,(int(np.round(x2)),int(np.round(y2))),5,(0,0,255),6)
        '''
        ...
        if key_points1[i] in kp_1 and key_points2[i] in kp_2:
        ...
        '''
    cv2.imwrite('/home/daizhuang/img1.jpg',img1)
