...
def Random_Projection_f(input_data,method="Sparse"):
    if method=="Gaussian":
        transformer=random_projection.GaussianRandomProjection()
    if method=="Sparse":
        transformer=random_projection.SparseRandomProjection(n_components=4096)
    New_data=transformer.fit_transform(input_data)
    return New_data 

###计算L2_dis_matrix，计算描述符的欧式距离#######
###des1:des1 is a numpy.array####
###des2:des2 is a numpy.array####
def compute_des_L2_dis(des1,des2):
    ####change numpy.array to torch.tensor and change cpu data to gpu data by use .cuda()
    des1=torch.from_numpy(des1).cuda(device)
...
###计算cos_dis_matrix，计算描述符的余弦距离#######
###des1:des1 is a numpy.array####
###des2:des2 is a numpy.array####
def compute_des_cos_dis(des1,des2):
    #des1=Random_Projection_f(des1)
    #des2=Random_Projection_f(des2)
...
####计算像素的欧式距离
def compute_pixel_L2_dis(des1,des2):
    ####change numpy.array to torch.tensor and change cpu data to gpu data by use .cuda()
    des1=torch.from_numpy(des1).cuda(device)
    des2=torch.from_numpy(des2).cuda(device)
    pdist=torch.nn.PairwiseDistance(2)
...
def compute_pixs_distance(des1,des2):
    L2_dis=compute_pixel_L2_dis(des1,des2)    ####L2_dis
    iterate_left=L2_dis.shape[0]
    ...
    for i in range(iterate_left):
        MIN_L2_DIS=np.min(L2_dis[i,:])   ####L2_dis
        M1[i]['i']=i
        M1[i]['j']=np.argmin(L2_dis[i,:])  ####L2_dis
        M1[i]['MAX']=MIN_L2_DIS
...
def match(img1,img2,kp1,kp2,max_match,H_path,imshow=False,method='cos'):
    if imshow:
        ...
        print('正确率为:'+str(p))
        show_keypoints(kp1,kp2,img1,img2,max_match,correct_match) 
    else:
        ...
        mAP=np.sum(AP)
        print("数据的AP为:"+str(mAP))
        return mAP

def show_keypoints(kp1,kp2,img1,img2,max_match,detect_correct_match):
    cols1 = img1.shape[1]
    ...
    for i in range(len(kp_1)):
        ...
        if kp_1[i] in key_points1 and kp_1[i] in key_points1:
    ...
    cv2.imwrite('/home/daizhuang/densenet169_match.jpg',img3)
